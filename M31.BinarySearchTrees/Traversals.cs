...
public static class Traversals
{
    ...
    public static void DepthFirstTraversal<T>(this Node<T>? tree, Action<Node<T>>? onEnter, Action<Node<T>>? onPass, Action<Node<T>>? onExit)
    {
        if (tree == null)
        {
            return;
    ...
    /// <summary>
    /// Depth-first search (DFS) traversal of the tree. Null-nodes are visited as well.
    /// </summary>
    /// <typeparam name="T">The type of the values.</typeparam>
    /// <param name="tree">The node tree.</param>
    /// <param name="onEnter">Action that will be executed when a node is entered.</param>
    ...
    /// between the traversals of the left and right subtrees.</param>
    /// <param name="onExit">Action that will be executed when a node is exited.</param>
    public static void DepthFirstTraversalNullNodesIncluded<T>(this Node<T>? tree, Action<Node<T>?>? onEnter, Action<Node<T>?>? onPass, Action<Node<T>?>? onExit)
    {
        onEnter?.Invoke(tree);
    ...
    /// <summary>
    /// Depth-first traversal of the tree. Makes use of a stack and yield returns one element at a time.
    /// This method redirects to <see cref="PreOrderTraversal{T}(Node{T}?)">PreOrderTraversal</see> and only exists for semantic reasons.
    /// </summary>
    /// <typeparam name="T">The type of the values.</typeparam>
    /// <param name="tree">The node tree.</param>
    /// <returns>The traversed nodes.</returns>
    public static IEnumerable<Node<T>> DepthFirstTraversal<T>(this Node<T>? tree)
    {
        return tree.PreOrderTraversal();
    }

    /// <summary>
    /// Breadth-first traversal of the tree. Makes use of a queue and yield returns one element at a time.
    /// </summary>
    /// <typeparam name="T">The type of the values.</typeparam>
    /// <param name="tree">The node tree.</param>
    /// <returns>The traversed nodes.</returns>
    public static IEnumerable<Node<T>> BreadthFirstTraversal<T>(this Node<T>? tree)
    {
        if (tree == null)
        {
            yield break;
